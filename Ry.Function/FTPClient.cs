...
namespace Ry.Function
{
    ...
    public class FTPClient
    {
        ...
        public void Connect()
        {
            ...
            {
                throw new IOException("FTP连接错误：无法连接指定的FTP服务器，" + RemoteHost + ":" + strRemotePort);
            }
            // 获取应答码
            ReadReply();
            if (iReplyCode != 220)
            ...
            if (!(iReplyCode == 331 || iReplyCode == 230))
            {
                ...
                throw new IOException("设置FTP登录用户时出错，" + strRemoteUser + "\r\n" + strReply.Substring(4));
            }
            if (iReplyCode != 230)
            {
                ...
                if (!(iReplyCode == 230 || iReplyCode == 202))
                {
                    ...
                    throw new IOException("设置FTP登录密码时出错，" + strRemotePass + "\r\n" + strReply.Substring(4));
                }
                else
                {
        ...
        public void Get(string strRemoteFileName, string
      strFolder, string strLocalFileName)
        {
            if (!bConnected)
            {
                Connect();
        ...
        //public void PrepareDir(string strDirName, bool create)
        //{
        //    string[] sDir = strDirName.Split('\\');
        //    string baseDir = strDirName.Substring(0, strDirName.IndexOf('\\', 2));
        //    string mDir = baseDir;
        //    bool isCheck = false;
        ...
        /// <summary>
        /// 检查并创建目录,并且传入的时候必须有文件名称，定位到该文件夹。
        /// 目录以\\分割
        /// </summary>
        /// <param name="strDirName">新的工作目录名</param>
        public void PrepareDir(string strDirName,bool created,bool changed)
        {
            string[] sDir = strDirName.Split('\\');
            bool isCheck = false;
        ...
        public void ChDir(string strDirName)
        {
            ...
            if (iReplyCode != 250)
            {
                throw new IOException("进入FTP指定路径时出错！" + strDirName  + "\r\n" + strReply.Substring(4));
            }
            this.strRemotePath = strDirName;
        }
